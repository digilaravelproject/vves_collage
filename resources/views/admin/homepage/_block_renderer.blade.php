        ...
        <div class="relative p-4 transition border rounded-lg bg-gray-50 hover:shadow-md group" :data-id="block.id">
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="flex gap-2">
                                <input type="text" x-model="block.image" @input="pushHistoryDebounced"
                                    class="relative z-10 flex-1 p-2 border rounded cursor-text" placeholder="https://...">
            ...
            <template x-if="block.type === 'instagram_profiles'">
                <div class="space-y-4">
                    ...
                    <div class="p-4 space-y-3 border-2 border-dashed border-pink-200 rounded-lg bg-linear-to-br from-pink-50/50 to-purple-50/50">
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-2">
                                        <input type="text" x-model="profile.image" @input="pushHistoryDebounced"
                                            class="flex-1 p-2 text-sm border border-gray-200 rounded-lg focus:border-pink-400 focus:ring-1 focus:ring-pink-200 outline-none transition" placeholder="https://...">
            ...
            <template x-if="block.type === 'institutions'">
                <div class="grid grid-cols-1 gap-3">
                    <div>
                        ...
                        <input type="text" x-model="block.section_title" @input="pushHistoryDebounced"
                            class="relative z-10 w-full p-2 border rounded cursor-text" placeholder="e.g., Our Educational Centers">
                    </div>
                </div>
            </template>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                                        ...
                                        <div class="relative aspect-square bg-gray-100 rounded border border-dashed flex items-center justify-center overflow-hidden group/img-upload cursor-pointer"
                                            ...
                                            <template x-if="item.photo">
                                                <img :src="item.photo" class="w-full h-full object-cover transition-transform duration-300 group-hover/img-upload:scale-105">
                                            </template>
                                            <template x-if="!item.photo">
                                                <div class="flex flex-col items-center gap-1">
                                                    <i class="bi bi-person-bounding-box text-2xl text-gray-300"></i>
                                                    <span class="text-[10px] text-gray-400 font-bold uppercase">Click to Upload</span>
                                                </div>
                                            </template>
                                            
                                            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover/img-upload:opacity-100 transition-opacity flex items-center justify-center">
                                                <i class="bi bi-camera-fill text-white text-xl"></i>
                                            </div>

                                            <input type="file" x-ref="advisorFile" class="hidden"
                                                @change="
                                                    const file = $event.target.files[0];
                                                    if(file){
                                                        const formData = new FormData();
                                    ...
                                    <div class="md:col-span-9 space-y-2">
                                        <input type="text" x-model="item.name" @input="pushHistoryDebounced"
                                            class="w-full p-2 text-sm border rounded focus:border-blue-400 outline-none" placeholder="Name">
                                        <textarea x-model="item.description" @input="pushHistoryDebounced" rows="2"
                                            class="w-full p-2 text-sm border rounded focus:border-blue-400 outline-none" placeholder="Short Bio"></textarea>
                                    </div>
                                </div>
                            </div>
                        </template>
                                            ...
                                            <div class="relative p-4 transition border rounded-lg bg-gray-50 hover:shadow-md group"
                                                    ...
                                                    <div class="space-y-4">
                                                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                                            ...
                                                            <div class="space-y-1">
                                                                 <label class="text-sm font-medium text-gray-600">Image</label>
                                                                 <div class="flex gap-2">
                                                                     <input type="text" x-model="childBlock.image" @input="pushHistoryDebounced"
                                                                         class="relative z-10 flex-1 p-2 border rounded cursor-text" placeholder="https://...">
                                                                     <button type="button" @click="$refs.introFileChild.click()" 
                                                                         class="px-3 bg-blue-50 text-blue-600 border border-blue-200 rounded hover:bg-blue-100 transition-colors">
                                                                         <i class="bi bi-upload"></i>
                                                                     </button>
                                                                     <input type="file" x-ref="introFileChild" class="hidden" @change="
                                                                         const file = $event.target.files[0];
                                                                         if(file){
                                                                             const formData = new FormData();
                                                                             formData.append('file', file);
                                                ...
                                                <template x-if="childBlock.type === 'instagram_profiles'">
                                                    <div class="space-y-4">
                                                        ...
                                                        <div class="p-3 space-y-3 border-2 border-dashed border-pink-200 rounded-lg bg-linear-to-br from-pink-50/50 to-purple-50/50">
                                                    ...
                                                    <div class="space-y-4">
                                                        ...
                                                        <div class="p-2 space-y-2 border-2 border-dashed border-blue-100 rounded-lg bg-blue-50/30">
                                                            <template x-for="(item, idx) in childBlock.items" :key="idx">
                                                                <div class="p-2 bg-white border border-gray-200 rounded shadow-sm relative flex gap-2">
                                                                    <button @click="block.columns[colIndex].blocks[childIndex].items.splice(idx, 1); pushHistory();"
                                                                    ...
                                                                    <div class="w-10 h-10 shrink-0 bg-gray-100 border rounded cursor-pointer flex items-center justify-center overflow-hidden group/child-img relative"
                                                                         ...
                                                                         <div class="absolute inset-0 bg-black/20 opacity-0 group-hover/child-img:opacity-100 transition-opacity flex items-center justify-center">
                                                                            <i class="bi bi-upload text-white text-xs"></i>
                                                                         </div>

                                                                         <input type="file" x-ref="advisorFileChild" class="hidden" 
                                                                            @change="
                                                                                const file = $event.target.files[0];
                                                                                if(file){
                                                                                    const formData = new FormData();
                                                                    ...
                                                                    <div class="flex-1 space-y-1">
                                                                        <input type="text" x-model="item.name" @input="pushHistoryDebounced"
                                                                            class="w-full p-1 text-[10px] border rounded" placeholder="Name">
                                                                        <textarea x-model="item.description" @input="pushHistoryDebounced" rows="1"
                                                                            class="w-full p-1 text-[10px] border rounded" placeholder="Bio"></textarea>
                                                                    </div>
                                                                </div>
                                                            </template>
                                                            <button @click="block.columns[colIndex].blocks[childIndex].items.push({ name: '', photo: '', description: '' }); pushHistory();"
                                                ...
                                                <template x-if="childBlock.type === 'institutions'">
                                                    <div class="grid grid-cols-1 gap-3">
                                                        <div>
                                                            <label class="text-xs font-bold text-gray-600 uppercase">Section Title (Optional)</label>
                                                            <input type="text" x-model="block.columns[colIndex].blocks[childIndex].section_title" @input="pushHistoryDebounced"
                                                                class="relative z-10 w-full p-2 text-sm border rounded cursor-text" placeholder="e.g., Our Educational Centers">
                                                        </div>
                                                    </div>
                                                </template>
