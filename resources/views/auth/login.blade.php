        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <div>
                <div class="flex items-center justify-between mb-2 px-1">
                    ...
                    @if (Route::has('password.request'))
                        <a class="text-[10px] font-bold text-(--primary-color) hover:text-(--primary-hover) transition-colors uppercase tracking-wider" href="{{ route('password.request') }}">
                            Recover?
                        </a>
                    @endif
                </div>
            ...
            <div class="flex items-center px-1">
                <label for="remember_me" class="inline-flex items-center cursor-pointer select-none">
                    <input id="remember_me" type="checkbox" class="w-4 h-4 text-(--primary-color) bg-gray-100 border-gray-300 rounded-lg focus:ring-(--primary-color)/10 focus:ring-2 transition-all cursor-pointer" name="remember">
