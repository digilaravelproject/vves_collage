<div x-data="eventTabs({{ $eventCategories->toJson() }}, {{ $items->toJson() }})" x-init="init()"
    ...
    <div class="mb-0 text-center" data-aos="fade-up" data-aos-duration="800">
        ...
        @if ($description)
            <p class="max-w-3xl px-4 mx-auto text-base font-normal leading-relaxed text-gray-600 mb-8">
                {{ $description }}
            </p>
        @endif
    </div>
        ...
        <div
            ...
            <template x-for="(category, index) in eventCategories" :key="category.id">
                <button @click="setActiveCategory(category.id)" :data-aos="'fade-up'" :data-aos-delay="index * 100"
                    class="px-1 py-2 sm:px-2 sm:py-2.5 rounded-full text-xs sm:text-base uppercase font-medium transition-all duration-300 ease-in-out border-2 whitespace-nowrap shrink-0"
                    :class="{
                            'border-vves-primary text-vves-primary bg-vves-primary/5 font-bold shadow-sm': activeCategoryId == category.id,
                            'border-transparent text-gray-500 hover:text-vves-primary hover:bg-gray-50': activeCategoryId != category.id
                        }">
    ...
    <div class="flex items-center justify-center">
        ...
        <div x-show="maxPages > 1" class="hidden pr-4 lg:block">
            <button @click="prevPage()" :disabled="currentPage === 1"
                class="p-3 text-white transition-colors duration-300 rounded-full bg-vves-primary disabled:bg-gray-400 hover:bg-blue-800">
                ...
                <template x-for="(item, index) in paginatedEvents" :key="item.id">
                    <div class="flex flex-col overflow-hidden transition-all duration-300 bg-white border border-gray-200 rounded-none shadow-lg hover:shadow-2xl hover:-translate-y-1 group"
                        ...
                        <a :href="item.link || '#'" class="relative block overflow-hidden aspect-square">
                            ...
                            <template x-if="!item.image_url">
                                <div class="flex items-center justify-center w-full h-full p-6 text-center text-white bg-linear-to-br from-vves-primary to-[#012740] transition-transform duration-700 group-hover:scale-105">
                        ...
                        <div class="relative flex flex-col flex-1 p-5">
                            <p class="text-sm font-bold text-vves-primary mb-2 uppercase tracking-wide"
                                x-text="item.formatted_date"></p>
                            <h3 class="mb-2 text-xl font-bold leading-tight text-gray-900">
                                <a :href="item.link || '#'" class="hover:text-vves-primary transition-colors"
                                    x-text="item.title"></a>
                            </h3>
                            <div class="flex items-center pt-3 mt-auto text-sm text-gray-600 border-t border-gray-100">
                                <svg class="w-4 h-4 mr-2 text-vves-primary" fill="none" stroke="currentColor" stroke-width="2"
                                    viewBox="0 0 24 24">
    ...
    <div x-show="maxPages > 1" class="flex justify-center mt-6 space-x-4 lg:hidden" data-aos="fade-up" data-aos-delay="200">
        <button @click="prevPage()" :disabled="currentPage === 1"
            class="p-2 text-white transition-colors duration-300 rounded-full bg-vves-primary disabled:bg-gray-400 hover:bg-blue-800">
