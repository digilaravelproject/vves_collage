...
<a href="{{ $notification->href ?: '#' }}" target="_blank" rel="noopener noreferrer"
    ...
    <div
        class="shrink-0 w-10 h-10 flex items-center justify-center bg-gray-100 text-blue-600 rounded-lg group-hover:bg-blue-600 group-hover:text-white transition-all duration-300">
        {{-- Icon Size: text-xl (chhota kar diya) --}}
        <span class="text-xl">{{ $notification->icon }}</span>
    </div>

    {{-- Title and Date --}}
    <div class="grow min-w-0"> {{-- min-w-0 zaroori hai text ko wrap karne ke liye --}}
    ...
    <div
        class="shrink-0 text-blue-600 opacity-0 group-hover:opacity-100 transform -translate-x-3 group-hover:translate-x-0 transition-all duration-300">
