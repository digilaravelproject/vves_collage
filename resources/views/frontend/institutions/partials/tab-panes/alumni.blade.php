...
<div id="pane-alumni" x-show="activeTab === 'alumni'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
    {{-- Header --}}
        ...
        @if(!empty($students))
            <section>
                <div class="flex items-center justify-between mb-10">
                    <div>
                        <span class="text-[10px] font-black text-blue-500 uppercase tracking-[0.2em] mb-2 block">Our Pride</span>
                        <h3 class="text-2xl md:text-3xl font-black text-[#1E234B] uppercase tracking-tight">Notable Students</h3>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($students as $student)
                        ...
                        <div class="group relative bg-white rounded-[32px] p-8 border border-gray-100 shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
                            <div class="absolute top-0 right-0 p-6 opacity-5 group-hover:opacity-10 transition-opacity">
                                <i class="bi bi-quote text-6xl text-[#000165]"></i>
                            </div>
                            
                            <div class="relative z-10">
                                <div class="w-20 h-20 rounded-2xl overflow-hidden mb-6 border-2 border-white shadow-xl rotate-3 group-hover:rotate-0 transition-transform duration-500">
                                ...
                                <h4 class="text-xl font-black text-[#1E234B] mb-1 group-hover:text-[#000165] transition-colors">{{ $student['name'] ?? 'N/A' }}</h4>
                                <div class="flex flex-wrap gap-2 mb-4">
                                    @if(!empty($student['batch']))
                                        <span class="px-2 py-0.5 bg-blue-50 text-blue-600 rounded text-[9px] font-black uppercase tracking-widest">Batch {{ $student['batch'] }}</span>
                                    @endif
                                    @if(!empty($student['section']))
                                        <span class="px-2 py-0.5 bg-gray-50 text-gray-500 rounded text-[9px] font-black uppercase tracking-widest">{{ $student['section'] }}</span>
                                    @endif
                                </div>
                                ...
                                <div class="space-y-3 pt-4 border-t border-gray-50">
                                    @if(!empty($student['profession']))
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-[#1E234B] text-xs">
        ...
        @if(!empty($testimonials))
            <section class="py-12 bg-gray-50/50 rounded-[60px] px-8 md:px-12 border border-gray-100">
                <div class="text-center mb-16">
                    <span class="text-[10px] font-black text-[#FFD700] uppercase tracking-[0.3em] mb-3 block">Voices of success</span>
                    <h3 class="text-3xl md:text-4xl font-black text-[#1E234B] uppercase tracking-tight">Alumni Testimonials</h3>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    @foreach($testimonials as $testimonial)
                        ...
                        <div class="bg-white p-10 rounded-[40px] shadow-sm border border-gray-50 relative group">
                            <div class="absolute -top-6 -right-6 w-12 h-12 bg-[#FFD700] rounded-full flex items-center justify-center text-[#000165] text-xl shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-500 scale-0 group-hover:scale-100">
                                <i class="bi bi-chat-heart-fill"></i>
                            </div>
                            
                            <div class="flex items-start gap-6">
                                <div class="shrink-0 w-16 h-16 rounded-full overflow-hidden border-2 border-[#000165]/10 p-1 shadow-inner">
        ...
        @if(!empty($gallery))
            <section>
                <div class="flex items-center gap-6 mb-12">
                    <h3 class="text-2xl md:text-3xl font-black text-[#1E234B] uppercase tracking-tight shrink-0">Alumni Meet Gallery</h3>
