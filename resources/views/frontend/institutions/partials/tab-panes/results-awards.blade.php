...
@if (!empty($resultsAwards) && is_array($resultsAwards))
                                    ...
                                    @foreach ($sectionItems as $item)
                                        ...
                                        @if (($item['type'] ?? 'result') === 'result')
                                            ...
                                            <div class="bg-white border border-gray-100 rounded-[32px] overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 group" x-data="{ expanded: false }">
                                                ...
                                                @php $students = $item['students'] ?? []; @endphp
                                                            ...
                                                            @foreach ($students as $student)
                                                                ...
                                                                <div class="flex items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-[#FFD700] transition-all duration-300 group/student">
                                                                    <div class="shrink-0 w-16 h-16 rounded-xl overflow-hidden border border-gray-100 shadow-sm relative">
                                                                        <img src="{{ !empty($student['photo']) ? asset('storage/' . $student['photo']) : 'https://ui-avatars.com/api/?name=' . urlencode($student['name'] ?? 'Student') . '&size=100&background=000165&color=fff' }}"
                                                                    ...
                                                                    <div class="min-w-0 flex-1">
                                                                        <h5 class="text-sm font-black text-[#1E234B] truncate" title="{{ $student['name'] ?? 'N/A' }}">{{ $student['name'] ?? 'N/A' }}</h5>
                                                                        <p class="text-[9px] font-bold text-gray-400 uppercase tracking-tight mb-1">{{ $student['class'] ?? 'Student' }}</p>
                                                                        <p class="text-xl font-black text-[#1E234B] group-hover/student:text-[#FFD700] transition-colors drop-shadow-sm leading-none">{{ $student['percentage'] ?? 'N/A' }}</p>
                                                                    </div>
                                                                </div>
                                                            @endforeach
                                                        </div>
                                                ...
                                                <div class="flex flex-col md:flex-row">
                                                    ...
                                                    <div class="flex-1 p-8 md:p-10">
                                                        <div class="flex items-center gap-3 mb-4">
                                                            <span class="w-8 h-8 rounded-full bg-[#FFD700]/10 text-[#1E234B] group-hover:text-[#FFD700] transition-colors flex items-center justify-center">
