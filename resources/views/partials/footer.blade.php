        ...
        <div class="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-4 lg:gap-14">
                        ...
                        @if ($val = setting($social['id']))
                            <a href="{{ $val }}" target="_blank" rel="noopener" 
                               ...
                               aria-label="{{ ucfirst(str_replace('_url', '', $social['id'])) }}">
                                ...
                                <i class="bi {{ $social['icon'] }} relative z-10 text-[14px] text-white transition-transform group-hover:scale-110"></i>
                            </a>
                        @endif
                    @endforeach
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <ul class="grid grid-cols-1 gap-x-4 gap-y-2.5 md:grid-cols-1">
                        @foreach ($topMenus as $item)
                            <li>
                                <a href="{{ $item->link }}" class="group flex items-center gap-2.5 text-[12px] text-white/70 hover:text-white transition-all font-semibold">
                                    <i class="bi bi-arrow-right-short text-lg text-[#FFD700]/50 group-hover:text-[#FFD700]"></i>
                ...
                <div class="space-y-4">
                    ...
                    <ul class="grid grid-cols-1 gap-x-4 gap-y-2.5">
                        @forelse ($footerLinks as $fl)
                            @if (!empty($fl['title']))
                                <li>
                                    <a href="{{ $fl['url'] }}" target="_blank" class="group flex items-center gap-2.5 text-[12px] text-white/70 hover:text-white transition-all font-semibold">
            ...
            <div class="lg:col-span-2 space-y-8" data-aos="fade-up" data-aos-delay="200">
                    ...
                    @forelse($contactCenters as $center)
                        <div class="space-y-5">
                            @if(!empty($center['name']))
                                <h5 class="text-white text-[13px] font-black tracking-tight leading-tight">{{ $center['name'] }}</h5>
                            @endif
                            ...
                            <div class="space-y-3.5 text-[12px]">
                                ...
                                @if (!empty($center['phone']))
                                    <div class="flex items-center gap-3">
                                        ...
                                        <a href="tel:{{ preg_replace('/\s+/', '', $center['phone']) }}" class="text-white font-bold hover:text-[#FFD700] transition-colors">{{ $center['phone'] }}</a>
                                    </div>
                                @endif
                                ...
                                @if (!empty($center['email']))
                                    <div class="flex items-center gap-3">
                                        ...
                                        <a href="mailto:{{ $center['email'] }}" class="text-white font-bold hover:text-[#FFD700] transition-colors break-all">{{ $center['email'] }}</a>
                                    </div>
                                @endif
                                ...
                                @if (!empty($center['website']))
                                    <div class="flex items-center gap-3">
                                        ...
                                        <a href="{{ $center['website'] }}" target="_blank" class="text-white font-bold hover:text-[#FFD700] transition-colors">Website</a>
                                    </div>
                                @endif
                            </div>
                        </div>
                ...
                <div class="relative group">
                    <div class="overflow-hidden rounded-3xl h-48 md:h-56 ring-1 ring-white/10 bg-white/5 border border-white/10 shadow-2xl">
                        @if($mapUrl)
                            ...
                            {{-- Directions Overlay --}}
                            <div class="absolute top-4 right-4 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                        ...
                        @else
                            <div class="flex items-center justify-center w-full h-full text-xs text-white/30">Google Map is not configured</div>
                        @endif
                    </div>
                </div>
            </div>
