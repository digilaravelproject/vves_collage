    ...
    if (!function_exists('renderDesktopRecursive')) {
        function renderDesktopRecursive($items, $level = 0) {
            ...
            foreach ($items as $item) {
                ...
                $html .= '<a href="' . $item->link . '" class="block text-xs font-normal text-gray-600 hover:text-theme hover:underline transition duration-150 wrap-break-word whitespace-normal" style="padding-left: ' . $paddingLeft . 'px;">' . $title . '</a>';
    ...
    if (!function_exists('renderMobileRecursive')) {
        function renderMobileRecursive($items, $level = 0)
        {
            ...
            foreach ($items as $item) {
                ...
                $html .= '<a href="' . $link . '" class="flex-1 py-2.5 pl-3 text-sm font-medium text-theme hover:bg-theme-light rounded-l-md transition">' . $title . '</a>';
                if ($hasChildren) {
                    $html .= '<button @click="open = !open" class="p-2.5 text-theme hover:bg-theme-light rounded-r-md transition">';
...
<style>
    /* ========================================================
       ...
       ======================================================== */
    :root {
        --theme-color: var(--primary-color, #000165);       /* Main Color */
        --theme-hover: var(--primary-hover, #00014d);       /* Darker Hover Color */
    }
    ...
    .hover\:bg-theme-dark:hover { background-color: var(--theme-hover) !important; }
    .border-theme { border-color: var(--theme-color) !important; }
    .border-t-theme { border-top-color: var(--theme-color) !important; }
    .border-l-theme { border-left-color: var(--theme-color) !important; }

    /* Auto-generates a light transparent version of the main color for hovers/dropdowns */
    .bg-theme-light { background-color: color-mix(in srgb, var(--theme-color) 8%, transparent) !important; }
    .hover\:bg-theme-light:hover { background-color: color-mix(in srgb, var(--theme-color) 12%, transparent) !important; }
    ...
    .marquee-track { display: inline-flex; gap: 3rem; animation: marquee 60s linear infinite; will-change: transform; }
    .marquee-track:hover { animation-play-state: paused; }
    .marquee-track span { display: inline-block; white-space: nowrap; }
    .marquee-link { color: #fdf2f8; text-decoration: underline; font-weight: 500; transition: opacity 0.3s; }
    .marquee-link:hover { opacity: 0.8; }
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between w-full h-full gap-2 md:gap-4 overflow-hidden py-1">
            ...
            <div class="flex items-center gap-3 w-full md:w-auto flex-1 overflow-hidden">
                ...
                <div class="hidden xl:flex items-center gap-2 shrink-0 border-l border-white/20 pl-3" x-data="{
                    ...
                    <button @click="toggleMusic()" class="flex items-center gap-1.5 focus:outline-none hover:text-gray-200 transition-colors">
                        <svg x-show="!playing" class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                        <svg x-show="playing" x-cloak class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>
                    ...
                    @if($collegeSongLyrics)
                        <a href="{{ $collegeSongLyrics }}" target="_blank" class="text-[10px] font-medium hover:underline text-white/80">(Lyrics)</a>
                    @endif
                </div>
            </div>
    ...
    <div class="w-full bg-white shadow-md flex items-center justify-between h-16 lg:h-20 pl-[110px] md:pl-[160px] lg:pl-[220px] pr-4 lg:pr-8 relative z-30">
        ...
        <div class="hidden lg:flex items-center h-full flex-1 justify-start overflow-hidden">
            <ul class="flex items-center h-full gap-3 xl:gap-5 whitespace-nowrap overflow-x-auto thin-scrollbar pb-1 pt-1">
                @foreach ($menus as $menu)
                    ...
                    <li x-data="{ openSub: false, activeTabIndex: 0 }" class="h-full flex items-center shrink-0 {{ $isStandardDropdown ? 'relative' : 'static' }}">

                        <a href="{{ $menu->link }}"
                           @if ($hasChildren) @mouseenter="openSub = true" @mouseleave="openSub = false" @endif
                           class="flex items-center gap-1 h-full text-[13px] xl:text-[14px] font-medium transition duration-200
                                  {{ $isActive ? 'text-theme' : 'text-gray-800 hover:text-theme' }}">

                            {{ getMenuLabel($menu->title) }}

                            @if ($hasChildren)
                                <svg class="w-3.5 h-3.5 mt-0.5 transition-transform duration-200 {{ $isActive ? 'text-theme' : 'text-gray-500' }}" :class="openSub ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            @endif
                        </a>
                        ...
                        @if ($isMegaMenu)
                            <div x-show="openSub" x-cloak
                                ...
                                <div class="mx-auto overflow-hidden bg-white shadow-xl max-w-[1250px] border-b border-b-gray-200 rounded-b-lg border-t-2 border-t-theme">
                                    <div class="flex">
                                        <!-- Level 2 Tabs -->
                                                ...
                                                @foreach ($menu->children as $index => $tabItem)
                                                    <li>
                                                        <a href="{{ $tabItem->link }}" @mouseenter="activeTabIndex = {{ $index }}"
                                                           :class="activeTabIndex === {{ $index }} ? 'bg-white text-theme font-semibold shadow-sm border-l-4 border-l-theme' : 'text-gray-700 hover:bg-gray-100 border-l-4 border-l-transparent'"
                                                           class="block transition-all duration-200 cursor-pointer px-4 py-2.5 text-sm">
                                                            {{ getMenuLabel($tabItem->title) }}
                                                            <span x-show="activeTabIndex === {{ $index }}" class="float-right text-theme">
                                                        ...
                                                        @foreach ($tabItem->children as $sub)
                                                            @if($sub->children->count() > 0)
                                                                <div class="flex flex-col w-full whitespace-normal">
                                                                    <a href="{{ $sub->link }}" class="block mb-2 text-sm font-bold text-gray-800 border-b-2 border-gray-100 pb-1 hover:text-theme hover:border-gray-300 transition-colors">
                                                            ...
                                                            @else
                                                                <div class="py-1 whitespace-normal">
                                                                    <a href="{{ $sub->link }}" class="block text-sm font-normal text-gray-700 hover:text-theme hover:translate-x-1 transition-transform duration-200">
                                                                        {{ getMenuLabel($sub->title) }}
                                                                    </a>
                                                                </div>
                                                            @endif
                                    ...
                                    <ul class="flex flex-col py-1">
                                        @foreach ($menu->children as $child)
                                            <li x-data="{ openChild: false }" @mouseenter="openChild = true" @mouseleave="openChild = false" class="relative">
                                                <a href="{{ $child->link }}" class="flex items-center justify-between px-3 py-2 text-sm text-gray-700 hover:bg-theme-light hover:text-theme transition duration-150 rounded-md whitespace-normal">
                                                    <span>{{ getMenuLabel($child->title) }}</span>
                                                    @if ($child->children->count())
                                                        <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"></path></svg>
                                                    @endif
                                                </a>
                                                @if ($child->children->count())
                                                    <ul x-show="openChild" x-cloak
                                                        ...
                                                        @foreach ($child->children as $subchild)
                                                            <li>
                                                                <a href="{{ $subchild->link }}" class="block px-4 py-1.5 text-sm text-gray-700 hover:bg-theme-light hover:text-theme transition duration-150 rounded-md whitespace-normal">
        ...
        <div class="flex items-center gap-3 lg:gap-5 shrink-0">
            ...
            <div class="relative flex items-center" x-data="{
                ...
                <button @click="searchOpen = !searchOpen" class="text-gray-800 hover:text-theme transition-colors focus:outline-none">
                ...
                <div x-show="searchOpen" x-cloak
                    ...
                    <input type="text" x-model="query" @input="search()" placeholder="Search site..." x-ref="searchInput"
                    ...
                    <!-- Search Results -->
                    <div x-show="results.length > 0 && query.length >= 2" class="mt-2 max-h-60 overflow-y-auto thin-scrollbar">
                        <ul class="flex flex-col">
                            <template x-for="result in results">
                                <li>
                                    <a :href="result.link" class="block px-3 py-2 text-sm text-gray-700 hover:bg-theme-light hover:text-theme rounded-md transition-colors">
                    ...
                    <div x-show="query.length >= 2 && results.length === 0" class="mt-2 p-2 text-sm text-center text-gray-500">
                        No results found.
                    </div>
                </div>
            </div>
            ...
            <button @click="mobileMenuOpen = !mobileMenuOpen" class="lg:hidden text-gray-800 hover:text-theme transition focus:outline-none">
                <svg x-show="!mobileMenuOpen" class="w-6 h-6 md:w-7 md:h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16m-7 6h7" />
                </svg>
                <svg x-show="mobileMenuOpen" x-cloak class="w-6 h-6 md:w-7 md:h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @foreach ($menus as $menu)
                <li x-data="{ openSub: false }" class="border-b border-gray-50 last:border-0">
                    <div class="flex items-center justify-between w-full px-4 bg-white">
                        <a href="{{ $menu->link }}" class="flex-1 py-3 text-[15px] font-bold text-theme hover:bg-gray-50 transition">
                            {{ getMenuLabel($menu->title) }}
                        </a>
                        @if ($menu->children->count())
                            <button @click="openSub = !openSub" class="p-3 text-theme rounded-full transition bg-gray-50 hover:bg-theme-light">
                                <svg :class="openSub ? 'rotate-180' : ''" class="w-4 h-4 transition-transform duration-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
